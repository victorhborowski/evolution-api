...
import { Label } from 'whaileys/lib/Types/Label';
import { LabelAssociation } from 'whaileys/lib/Types/LabelAssociation';
import {
  CatalogCollection,
  decryptPollVote,
...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async sendMessage(
    ...
    // Classic buttons message (native WhatsApp buttons)
    ...
    // Classic list message (native WhatsApp list with sections)
  ...
  public async buttonMessage(data: SendButtonsDto) {
    ...
    if (hasReplyButtons) {
      ...
      // === CLASSIC NATIVE BUTTONS (whaileys/baileys classic format) ===
      // Uses the classic buttonsMessage format that renders native WhatsApp buttons
      const classicButtons = data.buttons.map((btn) => ({
        buttonId: btn.id || `btn-${Math.random().toString(36).substring(2, 9)}`,
        buttonText: { displayText: btn.displayText || '' },
        type: 1,
      }));

      // If we have a thumbnail, send buttons with image
      if (data?.thumbnailUrl) {
        const buttonMessage: any = {
          image: isURL(data.thumbnailUrl) ? { url: data.thumbnailUrl } : Buffer.from(data.thumbnailUrl, 'base64'),
        ...
        return await this.sendMessageWithTyping(data.number, buttonMessage, {
          delay: data?.delay,
          presence: 'composing',
          quoted: data?.quoted,
          mentionsEveryOne: data?.mentionsEveryOne,
  ...
  public async listMessage(data: SendListDto) {
    // === CLASSIC LIST MESSAGE (whaileys/baileys classic format) ===
    // Uses the classic listMessage format with sections that renders native WhatsApp lists
    const listMsg: any = {
